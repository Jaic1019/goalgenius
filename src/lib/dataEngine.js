 ...
 *
 * Strategy:
 * 1. On first load: if DB is empty → call API and import ALL matches (bootstrap)
 * 2. Every 60s: call API silently, only update what changed (scores/status/new matches)
 * 3. If API fails at any point: DB data remains, user sees no error
 * 4. Admin can manually override any score if API is wrong
 */
...
function parseApiMatch(m) {
  ...
  // Resolve team names and flags — API can nest them differently
...
export async function bootstrapFromAPI() {
  ...
  console.log(`[Bootstrap] ✅ Imported ${inserted} matches from API`)
...
// ── Sync — called every 60s — only updates what changed ───────
...
export async function syncFromAPI() {
  ...
  for (const raw of arr) {
    ...
    if (!existing) {
      // New match (knockout fixture just published by API)
      toUpsert.push({ ...m, match_time: m.match_time })
    } else {
      // Skip if admin manually set this score
...
export async function checkAPIHealth() {
  ...
  const { ok } = await apiFetch('/get/games')
