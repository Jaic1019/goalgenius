...
export function usePredictions() {
  ...
  async function save(matchId, homeScore, awayScore, winnerPick) {
    ...
    const validationError = validatePrediction(homeScore, awayScore, winnerPick)
    ...
    if (!error) {
      setPredictions(p => ({ ...p, [matchId]: { home_score: Number(homeScore), away_score: Number(awayScore), winner_pick: winnerPick } }))
    }
