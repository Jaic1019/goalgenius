...
export default function Groups() {
  ...
  const groupNames = useMemo(() => {
    const names = matches.filter(m => { const s=(m.group_stage||'').toLowerCase(); return s.includes('group')||s.includes('groupe') })
  ...
  return (
    <div className="groups-page page fade-up">
      ...
      <div className="group-tabs">
        {groupNames.map(g=><button key={g} className={`gtab ${(active||groupNames[0])===g?'active':''}`} onClick={()=>setActive(g)}>{g}</button>)}
      </div>
      {current && (
        <div className="group-content">
          <div className="st-table">
            <div className="st-head"><span>Équipe</span><span className="ar">J</span><span className="ar">G</span><span className="ar">N</span><span className="ar">P</span><span className="ar">BP</span><span className="ar">BC</span><span className="ar">DB</span><span className="ar st-pts-h">Pts</span></div>
            {standings.map((t,i)=>(
              <div key={t.name} className={`st-row ${i<2?'qualified':''}`}>
                <span className="st-team"><span className="st-rk">{i+1}</span><Flag url={t.flag} name={t.name}/><span className="st-tn">{t.name}</span>{i<2&&<span className="q-badge">Q</span>}</span>
                <span className="ar st-n">{t.p}</span><span className="ar st-n">{t.w}</span><span className="ar st-n">{t.d}</span><span className="ar st-n">{t.l}</span>
            ...
            <div className="gm-list">
              {groupMatches.map(m=>(
                <div key={m.id} className={`gm-row ${m.status}`}>
                  ...
                  <div className="gm-mid">{m.home_score!=null?<span className="gm-sc display">{m.home_score}–{m.away_score}</span>:<span className="gm-dt">{m.match_date?.slice(5)} {m.match_time?.slice(0,5)}</span>}<span className={`badge ${m.status==='live'?'badge-red':m.status==='finished'?'badge-gray':'badge-purple'}`} style={{fontSize:9}}>{m.status==='live'?'Live':m.status==='finished'?'Terminé':'Bientôt'}</span></div>
      ...
      {groupNames.length===0&&!loading&&<div className="empty-state"><div className="empty-icon">📊</div><p>Les groupes apparaîtront une fois les matchs chargés.</p></div>}
    </div>
  )
}
