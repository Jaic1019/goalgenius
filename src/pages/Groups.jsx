...
import { toParisTime } from '../lib/timeUtils'
import './Groups.css'
...
function isGroupStage(g) {
  ...
  const koKeywords = ['16e','huitième','quart','demi','semi','final','finale','3ème','place','r32','r16','qf','sf']
  if (koKeywords.some(k => s.includes(k))) return false
...
function calcStandings(matches, group) {
  ...
  const teams = {}
  for (const m of gm) {
    if (m.home_team && m.home_flag && !teams[m.home_team])
      teams[m.home_team] = { name:m.home_team, flag:m.home_flag, p:0,w:0,d:0,l:0,gf:0,ga:0,pts:0 }
    if (m.away_team && m.away_flag && !teams[m.away_team])
...
const KNOCKOUT_STAGES = [
  { key:'r32',   label:'16e de finale',            group_stage:'16e de finale' },
  { key:'r16',   label:'Huitièmes de finale',       group_stage:'Huitièmes de finale' },
  { key:'qf',    label:'Quarts de finale',          group_stage:'Quarts de finale' },
  { key:'sf',    label:'Demi-finales',              group_stage:'Demi-finales' },
  { key:'third', label:'Match pour la 3ème place',  group_stage:'Match pour la 3ème place' },
  { key:'final', label:'Finale',                    group_stage:'Finale' },
]

export default function Groups() {
  const { matches, stadiums, loading } = useMatches()
  ...
  const availableKO = useMemo(() => {
    return KNOCKOUT_STAGES.filter(s => matches.some(m => m.group_stage === s.group_stage))
  }, [matches])
        ...
        <div className="group-content">
          ...
          <div className="gm-block">
            ...
            <div className="gm-list">
              ...
              {matchList.map(m => {
                const isLabel = !m.home_flag
                const stadium  = stadiums[m.stadium_id]
                const tz       = stadium?.timezone || 'America/New_York'
                const timeInfo = toParisTime(m.local_date_raw, tz, m.match_date, m.match_time)
                const homeDisplay = isLabel ? (m.home_team_label || m.home_team || 'À déterminer') : m.home_team
                const awayDisplay = isLabel ? (m.away_team_label || m.away_team || 'À déterminer') : m.away_team
                return (
                  <div key={m.id} className={`gm-row ${m.status}${isLabel?' gm-tbd':''}`}>
                    <div className="gm-team home">
                      {!isLabel&&<Flag url={m.home_flag} name={homeDisplay}/>}
                    ...
                    <div className="gm-mid">
                      {m.home_score!=null
                        ...
                        :<span className="gm-dt">{timeInfo.full}</span>
                      }
                    ...
                    <div className="gm-team away">
                      ...
                      {!isLabel&&<Flag url={m.away_flag} name={awayDisplay}/>}
                    </div>
                  </div>
                )
              })}
