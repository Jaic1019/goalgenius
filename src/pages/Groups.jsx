...
// Only true group stage (Group A through Group L)
function isGroupStage(g) {
  if (!g) return false
  if (/^group [a-l]$/i.test(g.trim())) return true
  if (/^groupe [a-l]$/i.test(g.trim())) return true
  const knockouts = ['r32','r16','qf','quarter','sf','semi','demi','final','3rd','third']
  const s = g.toLowerCase()
  if (knockouts.some(k=>s.includes(k))) return false
  return s.includes('group')||s.includes('groupe')
}
...
export default function Groups() {
  ...
  const groupMatches = useMemo(()=>matches.filter(m=>m.group_stage===current)
    ...
    <div className="groups-page page fade-up">
      ...
      {current && standings.length>0 && (
        <div className="group-content">
          {/* Standings — team names always fully visible */}
                ...
                <div key={m.id} className={`gm-row ${m.status}`}>
                  ...
                  <div className="gm-mid">
                    {m.home_score!=null&&m.status==='finished'
                      ...
                      :m.status==='live'
                      ?<span className="gm-sc display gm-live">{m.home_score??0}–{m.away_score??0}</span>
